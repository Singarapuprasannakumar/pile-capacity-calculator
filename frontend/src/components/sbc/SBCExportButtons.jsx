...
import { exportSbcToExcel, exportSbcToCSV } from '../../utils/sbcTableExport';
import { generateSbcPDF } from '../../utils/sbcPdfReport';
import { FileDown, Download, Printer } from 'lucide-react';

export default function SBCExportButtons({ reportOrReports }) {
  ...
  const handleDownloadPDF = async () => {
    setPdfLoading(true);
    try {
      generateSbcPDF(reportOrReports);
    } catch (e) {
      console.error('Error generating SBC PDF:', e);
    } finally {
      setPdfLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-wrap gap-2.5 no-print">
      ...
      <button
        onClick={() => exportSbcToExcel(reportOrReports)}
      ...
      {/* CSV Download Button */}
      <button
        onClick={() => exportSbcToCSV(reportOrReports)}
