...
import { exportSoilToExcel, exportSoilToCSV } from '../../utils/soilTableExport';
import { generateSoilPDF } from '../../utils/soilPdfReport';
import { FileDown, Download, Printer } from 'lucide-react';

export default function SoilClassificationExportButtons({ reportOrReports }) {
  ...
  const handleDownloadPDF = async () => {
    setPdfLoading(true);
    try {
      generateSoilPDF(reportOrReports);
    } catch (e) {
      console.error('Error generating Soil Classification PDF:', e);
    } finally {
      setPdfLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-wrap gap-2.5 no-print">
      ...
      <button
        onClick={() => exportSoilToExcel(reportOrReports)}
      ...
      {/* CSV Download Button */}
      <button
        onClick={() => exportSoilToCSV(reportOrReports)}
